				...
				if (is_single() || is_attachment()) {
					...
					if ($cat) {
						...
						if ($level > $args['max_levels'])
                            ...
                            $rez_parent = '<a class="breadcrumbs_item cat_post" href="'.esc_url(get_term_link($cat->term_id)).'">'
...
if ( ! function_exists('frank_jewelry_store_get_privacy_text' ) ) {
	function frank_jewelry_store_get_privacy_text() {
		$page = get_option( 'wp_page_for_privacy_policy' );
		$privacy_text = frank_jewelry_store_get_theme_option( 'privacy_text' );
		return apply_filters( 'frank_jewelry_store_filter_privacy_text', wp_kses_post(
				$privacy_text
				. ( ! empty( $page ) && ! empty( $privacy_text )
					// Translators: Add url to the Privacy Policy page
					? ' ' . sprintf( __( 'For further details on handling user data, see our %s', 'frank-jewelry-store' ),
						'<a href="' . esc_url( get_permalink( $page ) ) . '" target="_blank">'
						. __( 'Privacy Policy', 'frank-jewelry-store' )
						. '</a>' )
					: ''
				)
			)
