...
if (!empty($frank_jewelry_store_logo_image) || !empty($frank_jewelry_store_logo_text)) {
	?><a class="logo" href="<?php echo is_front_page() ? '#' : esc_url(home_url('/')); ?>"><?php
		if (!empty($frank_jewelry_store_logo_image)) {
			...
			echo '<img src="'.esc_url($frank_jewelry_store_logo_image).'" class="logo_main" alt="'.esc_attr__('logo image', 'frank-jewelry-store').'"'.(!empty($frank_jewelry_store_attr[3]) ? sprintf(' %s', $frank_jewelry_store_attr[3]) : '').'>' ;
		} else {
