...
if ( !function_exists('frank_jewelry_store_init_override_options') ) {
	add_action( 'after_setup_theme', 'frank_jewelry_store_init_override_options' );
	function frank_jewelry_store_init_override_options() {
		if ( is_admin() ) {
			add_action("admin_enqueue_scripts", 'frank_jewelry_store_add_override_options_scripts');
			add_action('save_post',			'frank_jewelry_store_save_override_options');
			add_filter('trx_addons_filter_override_options',	'frank_jewelry_store_add_override_options');
		}
	}
}
	
// Load required styles and scripts for admin mode
if ( !function_exists( 'frank_jewelry_store_add_override_options_scripts' ) ) {
	add_action("admin_enqueue_scripts", 'frank_jewelry_store_add_override_options_scripts');
	function frank_jewelry_store_add_override_options_scripts() {
		// If current screen is 'Edit Page' - load fontello
		$screen = get_current_screen();
		if (frank_jewelry_store_allow_override_options($screen->id) && frank_jewelry_store_allow_override_options($screen->post_type)) {
			...
			frank_jewelry_store_enqueue_script( 'frank_jewelry_store-override-options', frank_jewelry_store_get_file_url('theme-options/theme.override-options.js'), array('jquery') );
			wp_localize_script( 'frank_jewelry_store-override-options', 'frank_jewelry_store_dependencies', frank_jewelry_store_get_theme_dependencies() );
		}
	}
}
...
if (!function_exists('frank_jewelry_store_allow_override_options')) {
	function frank_jewelry_store_allow_override_options($post_type) {
		return apply_filters('frank_jewelry_store_filter_allow_override_options', in_array($post_type, array('page', 'post')), $post_type);
	}
}


// Add override options
if (!function_exists('frank_jewelry_store_add_override_options')) {
	function frank_jewelry_store_add_override_options($boxes = array()) {
		global $post_type;
		if (frank_jewelry_store_allow_override_options($post_type)) {
			$boxes[] = array('id' => sprintf('frank_jewelry_store_override_options_%s', $post_type),
				'title' =>  esc_html__('Theme Options', 'frank-jewelry-store'),
				'callback' => 'frank_jewelry_store_show_override_options',
				'page' => $post_type,
				'context' => 'advanced',
				'priority' => 'default'
			);
...
if (!function_exists('frank_jewelry_store_show_override_options')) {
	function frank_jewelry_store_show_override_options() {
		...
		if (frank_jewelry_store_allow_override_options($post_type)) {
			...
			foreach ($FRANK_JEWELRY_STORE_STORAGE['options'] as $k=>$v) {
				...
				$v['val'] = isset($meta[$k]) ? $meta[$k] : 'inherit';
				$tabs_content[$v['override']['section']] .= frank_jewelry_store_show_override_options_field($k, $v);
			}
			if (count($tabs_titles) > 0) {
				?>
				<div class="frank_jewelry_store_override_options">
					...
					<div id="frank_jewelry_store_override_options_tabs">
						<ul><?php
							$cnt = 0;
							foreach ($tabs_titles as $k=>$v) {
								$cnt++;
								?><li><a href="#frank_jewelry_store_override_options_<?php echo esc_attr($cnt); ?>"><?php echo esc_html($v); ?></a></li><?php
							}
						?></ul>
						<?php
							$cnt = 0;
							foreach ($tabs_content as $k=>$v) {
								$cnt++;
								?>
								<div id="frank_jewelry_store_override_options_<?php echo esc_attr($cnt); ?>" class="frank_jewelry_store_override_options_section">
...
if ( !function_exists('frank_jewelry_store_show_override_options_field') ) {
	function frank_jewelry_store_show_override_options_field($name, $field) {
		$inherit_state = frank_jewelry_store_is_inherit($field['val']);
		$output = '<div class="frank_jewelry_store_override_options_item frank_jewelry_store_override_options_item_'.esc_attr($field['type']).' frank_jewelry_store_override_options_inherit_'.($inherit_state ? 'on' : 'off' ).'">'
						. '<h4 class="frank_jewelry_store_override_options_item_title">'
							. esc_html($field['title'])
							. '<span class="frank_jewelry_store_override_options_inherit_lock" id="frank_jewelry_store_override_options_inherit_'.esc_attr($name).'"></span>'
						. '</h4>'
						. '<div class="frank_jewelry_store_override_options_item_data">'
							. '<div class="frank_jewelry_store_override_options_item_field" data-param="'.esc_attr($name).'"'.(!empty($field['linked']) ? ' data-linked="'.esc_attr($field['linked']).'"' : '').'>';
		if ($field['type']=='checkbox') {
			$output .= '<label class="frank_jewelry_store_override_options_item_label">'
						. '<input type="checkbox" name="frank_jewelry_store_override_options_field_'.esc_attr($name).'" value="1"'.($field['val']==1 ? ' checked="checked"' : '').' />'
						. esc_html($field['title'])
					. '</label>';
		} else if ($field['type']=='switch' || $field['type']=='radio') {
			foreach ($field['options'] as $k=>$v) {
				$output .= '<label class="frank_jewelry_store_override_options_item_label">'
							. '<input type="radio" name="frank_jewelry_store_override_options_field_'.esc_attr($name).'" value="'.esc_attr($k).'"'.($field['val']==$k ? ' checked="checked"' : '').' />'
							. esc_html($v)
						. '</label>';
			}
		} else if ($field['type']=='text') {
			$output .= '<input type="text" name="frank_jewelry_store_override_options_field_'.esc_attr($name).'" value="'.esc_attr(frank_jewelry_store_is_inherit($field['val']) ? '' : $field['val']).'" />';
		} else if ($field['type']=='textarea') {
			$output .= '<textarea name="frank_jewelry_store_override_options_field_'.esc_attr($name).'">'.esc_html(frank_jewelry_store_is_inherit($field['val']) ? '' : $field['val']).'</textarea>';
		} else if ($field['type']=='select') {
			$output .= '<select size="1" name="frank_jewelry_store_override_options_field_'.esc_attr($name).'">';
			foreach ($field['options'] as $k=>$v) {
				$output .= '<option value="'.esc_attr($k).'"'.($field['val']==$k ? ' selected="selected"' : '').'>'.esc_html($v).'</option>';
			}
			$output .= '</select>';
		} else if (in_array($field['type'], array('image', 'media', 'video', 'audio'))) {
			$output .= '<input type="text" id="frank_jewelry_store_override_options_field_'.esc_attr($name).'" name="frank_jewelry_store_override_options_field_'.esc_attr($name).'" value="'.esc_attr(frank_jewelry_store_is_inherit($field['val']) ? '' : $field['val']).'" />'
					. frank_jewelry_store_show_custom_field('frank_jewelry_store_override_options_field_'.esc_attr($name).'_button', array(
														'type'				=> 'mediamanager',
														'data_type'			=> $field['type'],
														'linked_field_id'	=> 'frank_jewelry_store_override_options_field_'.esc_attr($name)),
														null)
					. '<div class="frank_jewelry_store_override_options_field_preview">'
						. (frank_jewelry_store_is_inherit($field['val']) ? '' : ($field['val'] && $field['type']=='image' ? '<img src="' . esc_url($field['val']) . '" alt="'.esc_attr__('image', 'frank-jewelry-store').'">' : basename($field['val'])))
					. '</div>';
		}
		$output .= '<div class="frank_jewelry_store_override_options_inherit_cover'.(!$inherit_state ? ' frank_jewelry_store_hidden' : '').'">'
							. '<span class="frank_jewelry_store_override_options_inherit_label">' . esc_html__('Inherit', 'frank-jewelry-store') . '</span>'
							. '<input type="hidden" name="frank_jewelry_store_override_options_inherit_'.esc_attr($name).'" value="'.esc_attr($inherit_state ? 'inherit' : '').'" />'
						. '</div>'
					. '</div>'
					. '<div class="frank_jewelry_store_override_options_item_description">'
...
if (!function_exists('frank_jewelry_store_save_override_options')) {
	//Handler of the add_action('save_post', 'frank_jewelry_store_save_override_options');
	function frank_jewelry_store_save_override_options($post_id) {

		// verify nonce
		if ( !wp_verify_nonce( frank_jewelry_store_get_value_gp('override_options_post_nonce'), admin_url() ) )
			return $post_id;
		...
		foreach ($options as $k=>$v) {
			...
			if (!empty($_POST['frank_jewelry_store_override_options_inherit_' . $k])) continue;
			// Get option value from POST
			$meta[$k] = isset($_POST['frank_jewelry_store_override_options_field_' . $k])
							? $_POST['frank_jewelry_store_override_options_field_' . $k]
							: ($v['type']=='checkbox' ? 0 : '');
		}
