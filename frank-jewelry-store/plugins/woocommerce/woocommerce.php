...
if (!function_exists('frank_jewelry_store_woocommerce_theme_setup9')) {
	...
	function frank_jewelry_store_woocommerce_theme_setup9() {
		...
		if (frank_jewelry_store_exists_woocommerce()) {
			...
			add_action(		'woocommerce_shop_loop_subcategory_title',  'frank_jewelry_store_woocommerce_shop_loop_subcategory_title', 9, 1);
...
if ( !function_exists( 'frank_jewelry_store_woocommerce_open_item_wrapper' ) ) {
	...
	function frank_jewelry_store_woocommerce_title_wrapper_start($cat='') {
				...
				<div class="mask"></div>
				<?php
				frank_jewelry_store_hovers_add_icons('shop',array('cat'=>$cat));
				?>
			</div><!-- /.post_featured -->
			<div class="post_data">
...
if ( !function_exists( 'frank_jewelry_store_woocommerce_title_tags' ) ) {
	...
	function frank_jewelry_store_woocommerce_title_tags() {
		global $product;
		frank_jewelry_store_show_layout(wc_get_product_tag_list( $product->get_id(), ', ', '<div class="post_tags product_tags">', '</div>' ));
	}
}
...
// Wrap product title into link
if ( !function_exists( 'frank_jewelry_store_woocommerce_the_title' ) ) {
	//add_filter( 'the_title', 'frank_jewelry_store_woocommerce_the_title' );
	function frank_jewelry_store_woocommerce_the_title($title) {
		if (frank_jewelry_store_storage_get('in_product_item') && get_post_type()=='product') {
...
// Wrap category title into link
if ( !function_exists( 'frank_jewelry_store_woocommerce_shop_loop_subcategory_title' ) ) {
    //Handler of the add_filter( 'woocommerce_shop_loop_subcategory_title', 'frank_jewelry_store_woocommerce_shop_loop_subcategory_title' );
    function frank_jewelry_store_woocommerce_shop_loop_subcategory_title($cat) {

        $cat->name = sprintf('<a href="%s">%s</a>', esc_url(get_term_link($cat->slug, 'product_cat')), $cat->name);
        ?>
        <h2 class="woocommerce-loop-category__title">
        <?php
        frank_jewelry_store_show_layout($cat->name);
        if ( $cat->count > 0 ) {
            echo apply_filters( 'woocommerce_subcategory_count_html', ' <mark class="count">(' . esc_html( $cat->count ) . ')</mark>', $cat ); // WPCS: XSS ok.
        }
        ?>
        </h2><?php
        //if (frank_jewelry_store_storage_get('in_product_item') && is_object($cat)) {
        //	$cat->name = sprintf('<a href="%s">%s</a>', esc_url(get_term_link($cat->slug, 'product_cat')), $cat->name);
        //}
        //return $cat;
    }
