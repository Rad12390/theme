...
function frank_jewelry_store_ready_actions() {
	...
	FRANK_JEWELRY_STORE_STORAGE['top_panel_height'] = 0;	//Math.max(0, jQuery('.top_panel_navi').height());
	...
	//jQuery("select:not(.esg-sorting-select):not(.woocommerce-currency-switcher)").wrap('<div class="select_container"></div>');
...
//		jQuery.cookie('frank_jewelry_store_shop_mode', mode, {expires: 365, path: '/'});
	...
	jQuery('.nav-links-more a').on('click', function(e) {
		...
		if (panel.length == 0) {															// Load simple page content
			jQuery.get(location.href, {
				...
				//frank_jewelry_store_loadmore_add_items(jQuery('.content > .posts_container').eq(0), jQuery(response).find('.content > .posts_container > article'));
	...
	if (FRANK_JEWELRY_STORE_STORAGE['use_mediaelements'] && cont.find('audio:not(.inited),video:not(.inited)').length > 0) {
		...
		if (window.mejs.MediaElementDefaults) window.mejs.MediaElementDefaults.enableAutosize = true;
			cont.find('audio:not(.inited),video:not(.inited)').each(function() {
				"use strict";
				if (jQuery(this).parents('.mejs-mediaelement').length == 0 && (FRANK_JEWELRY_STORE_STORAGE['init_all_mediaelements'] || (!jQuery(this).hasClass('wp-audio-shortcode') && !jQuery(this).hasClass('wp-video-shortcode') && !jQuery(this).parent().hasClass('wp-playlist')))) {
